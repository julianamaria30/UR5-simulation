...
def main():
    ...
    qf = np.array([0, 22.5 * RAD2DEG, 67.5 * RAD2DEG, 0, -90 * RAD2DEG, 0])  # Posição final das juntas (rad)
    ...
    # Enviar a posição final para o robô
    set_joint_positions(clientID, joint_handles, qf)
    vrep.simxSynchronousTrigger(clientID)  # Dispara a próxima etapa da simulação
    vrep.simxGetPingTime(clientID)  # Espera até a simulação ser concluída
